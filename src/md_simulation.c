...
void simulation( Polymers* Ply, Params parameters, double* r_init, int numPolymers, int simnum ){
	...
	//printf("---N_tot = %d\n", N_tot);
	char filenameR[sizeof "poly_exp1/R_XXX_XXX.dat"]; 
	sprintf(filenameR, "poly_exp1/R_%03d_%03d.xyz", N_tot, simnum);
	
	char filenameF[sizeof "poly_exp1/F_XXX_XXX.dat"]; 
	sprintf(filenameF, "poly_exp1/F_%03d_%03d.xyz", N_tot, simnum);
